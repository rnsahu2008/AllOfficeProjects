...
import com.lc.jsonasserts.JSONComparator;
...
public class JSONAssert {
    ...
    public static void assertEquals(JSONArray expected, JSONArray actual, JSONCompareMode compareMode)
        ...
        if (result.failed()) {
        	...
        	//ATUReports.add("assertEquals", Thread.currentThread().getStackTrace()[2].getMethodName().toString(), "", expected.toString(), actual.toString(), LogAs.FAILED, null, "", null);
            throw new AssertionError(result.getMessage());
        }
        else{
        	System.out.println("PASSED!!");
        	//ATUReports.add("assertEquals", Thread.currentThread().getStackTrace()[2].getMethodName().toString(), "", expected.toString(), actual.toString(), LogAs.PASSED, null, "", null);
        }
    }
